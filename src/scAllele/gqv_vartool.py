...
class variant:
	...
	def shift_Rpos(self, offset):
		for ri, (ri_s, ri_e) in self.ReadPos.items():
			self.ReadPos[ri] = (ri_s + offset, ri_e + offset)
	...
	def trim(self, first = "sufix", min_match = 3):
		Pfx, Sfx, ALT, REF = trim_sequences(self.ALT, self.REF, first = first, min_match = min_match)
		self.REF = REF 
		self.ALT = ALT
		self.g_start = self.g_start + Pfx
		self.g_end   = self.g_end - Sfx
		for read_name, (r_start, r_end) in self.ReadPos.items():
			self.ReadPos[read_name] = (r_start + Pfx, r_end - Sfx)		
	...
	def leftalign(self, REFPOS, max_shift = 100):
		max_shift = max(REFPOS.pos0 + 1, self.g_start - max_shift)
		...
		for RI, (r_start, r_end) in self.ReadPos.items():
			self.ReadPos[RI] = (r_start - offset, r_end - offset + 1)
...
def calculate_edit_distance(var, Junctions, REFPOS):
	...
	is_intron = False #; print("jxns", sorted(Junctions))
...
def edit_distance_function_msnp(var):
	...
	for i in range(var.L_ref()):
		snp_ReadPos = {}
		for r_name, (r_start, r_end) in var.ReadPos.items():
...
def edit_distance_function_internal_aln(var, Junctions, REFPOS):
	...
	while max_scores and top <= 3:
		...
		_ReadPos = {}
		for RI, (s, e) in var.ReadPos.items():
...
def count_tandem_repeat_around_variant(var, REFPOS):
	...
	for repLen in range(1, 5):
		for (l1, l2) in [(var.g_start, var.g_start + repLen), (var.g_start - repLen, var.g_start)]:
			...
			repCount_up = 0
			start_pos = l1
			while start_pos - repLen > REFPOS.pos0 and REFPOS.get_sequence(start_pos - repLen, start_pos) == repSeq:
				repCount_up += 1
				start_pos -= repLen

			repCount_dn = 0
			end_pos = l1
			while end_pos + repLen < REFPOS.end1 and REFPOS.get_sequence(end_pos, end_pos + repLen) == repSeq:
				repCount_dn += 1
				end_pos += repLen
	...
	REP_REGION = HTSeq.GenomicInterval(REFPOS.chrom, repeat_pos - 1, repeat_end + 1)
...
def trim_var_with_var(var1, var2):
	...
	if var2.g_start == var1.g_start:
		Pfx_ref, Sfx_ref, REF_12, ref_2c = trim_sequences(var1.REF, var2.REF, first = 'prefix')
		Pfx_alt, Sfx_alt, ALT_12, alt_2c = trim_sequences(var1.ALT, var2.ALT, first = 'prefix')

	elif var2.g_end == var1.g_end:
		Pfx_ref, Sfx_ref, REF_12, ref_2c = trim_sequences(var1.REF, var2.REF, first = 'sufix')
		Pfx_alt, Sfx_alt, ALT_12, alt_2c = trim_sequences(var1.ALT, var2.ALT, first = 'sufix')
	else:
		return None
	...
	_ReadPos = {}
	for r, (r_start, r_end) in var1.ReadPos.items():
...
def find_equivalent_indels(VAR_COLLECTION, REFPOS):
	...
	while i < len(VAR_COLLECTION):
		...
		if split_candidates:
			...
			if split_candidates[(variant_2, variant_3)]	< 100.1:
				...
				for RI, (r_start, r_end) in variant_1.ReadPos.items():
					_ReadPos[RI] = (r_start + Pfx, r_end - Sfx)
				variant_2.ReadPos.update(_ReadPos)
				VAR_COLLECTION[i] = None
		i += 1
	...
	for i, variant in enumerate(VAR_COLLECTION):
		if variant is None:
			...
			variant.rep_region = HTSeq.GenomicInterval(REFPOS.chrom, variant.g_start - 1, variant.g_end + 1)
			continue
		elif variant.get_variant_type() == "SNV":
			REP_COUNT, REP_REGION = count_tandem_repeat_around_variant(variant, REFPOS)
			variant.rep_region = HTSeq.GenomicInterval(REFPOS.chrom, variant.g_start, variant.g_end)
	...
	for i, variant_1 in enumerate(VAR_COLLECTION):
		...
		for j, variant_2 in enumerate(VAR_COLLECTION):
			...
			if variant_1.same_variant(variant_2, REFPOS):
				...
				VAR_COLLECTION[i].ReadPos.update(variant_2.ReadPos)
	...
	for i, variant in enumerate(VAR_COLLECTION):
		if variant is None:
			...
			if REFPOS.RC.start <= variant.g_start and variant.g_start <= REFPOS.RC.end:
				pass
			else:
				VAR_COLLECTION[i] = None
