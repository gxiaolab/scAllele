...
def construct_de_Bruijn_graph(Unique_Read_Seqs, k):
	G = nx.DiGraph()
...
def compress_read_paths(xdb, k, RP, SetOfReadGroups):
	...
	for PATH_str in sorted(tmp1_SetOfReadIndexes.keys(), key = lambda x : len(x), reverse = True):
		...
		while xdb.in_degree(PATH[0]) == 1 and xdb.predecessors(PATH[0]).__next__() not in PATH \
			and not any(special_char in xdb.predecessors(PATH[0]).__next__() for special_char in ["I", "X", "Y"]):
		...
		while xdb.out_degree(PATH[-1]) == 1 and xdb.successors(PATH[-1]).__next__() not in PATH \
			and not any(special_char not in xdb.successors(PATH[-1]).__next__() for special_char in ["I", "X", "Y"]):
	...
	for Read_Index, (PATH_str, Read_Group_Set) in enumerate(tmp2_SetOfReadIndexes.items()):
		...
		read_obj = Read_Index_Object(INDEX, LongSeq, k, Read_Group_Set)
...
class Read_Index_Object:
    ...
    def __init__(self, path_index, long_seq, k, Read_Group_Set):
    ...
    def find_position(self, REFPOS, POS, END):
...
def find_Source_Target_pairs(xdb, REF_Index, SetOfReadIndexes, k):
	OV = k - 2
...
def make_read_dbg(Read_Index, Ref_Index, SetOfReadIndexes, RC):
	...
	for Rid_iv in EVENTS:
		...
		for Ref_iv in EVENTS[Rid_iv]:
			...
			if Rid_iv.get_seq(Rid_Long_Seq) == Ref_iv.get_seq(Ref_Long_Seq):
				continue
...
def draw_rid_graph(xdb, graph_file_name):
	...
	for nd_or, nd_dt in xdb.edges, RC_i:
